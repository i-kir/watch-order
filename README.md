...
映画・アニメシリーズを観る順番を、**公開順・時系列順・おすすめ順**で比較できるサイトです。映画シリーズを基本とし、順番の問題がとくに強いアニメを加えています。
...
キーは `.env.local` に書いておくこともできます（`.gitignore` 済み）。その場合は毎回渡さずに済みます。

```
TMDB_API_KEY=32文字のキー
```

シリーズを足したら、このコマンドを1回流すだけです。
`content/series/` を自動で走査するので、ファイル名をどこかに登録する必要はありません。
取得済みの作品はスキップされます。
